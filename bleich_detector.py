...
def meddler(attack_values, s_start_after =0, is_calibration = False, is_second_oracle=False):#test_cnt = 20, repeat_cnt=REP_COUNT):
    ...
    if is_calibration:
        ...
        while True:#not found_fast_token:
            ...
            modifier_outfile.flush()
            res = my_attack_sim.find_multiplier(padding_oracle_ext_with_comparison_cur, c0, s_min+s_start_after, 600)#600)#600)00)#1000)#1010)
            ...
            if not res[2]:
                ...
                if (possible.count(0) > 2):
                    ...
                    to_write+= str(c0)+"\n"
                    res = my_attack_sim.find_multiplier(padding_oracle_ext_with_comparison_cur, c0, s_min+s_start_after+firstmul, 600-firstmul)#200-firstmul)#600-firstmul)#1000)#1010)
                    if res[2]:
                        continue
                    else:
                        check_again = padding_oracle_ext_with_comparison_cur.multiply(c0, res[0])
